using BlazorPeliculasNet7.Client.Helpers;
...
namespace BlazorPeliculasNet7.Client.Auth
{
    public class ProveedorAutenticacionJWT : AuthenticationStateProvider, ILoginService
    {
        ...
        public async Task Login(UserTokenDTO tokeDTO)
        {
            await js.GuardarEnLocalStorage(TOKENKEY, tokeDTO.Token);
            await js.GuardarEnLocalStorage(EXPIRATIONTOKENKEY, tokeDTO.Expiration.ToString());
            var authState = ConstruirAuthenticationState(tokeDTO.Token);
            NotifyAuthenticationStateChanged(Task.FromResult(authState));
        }
